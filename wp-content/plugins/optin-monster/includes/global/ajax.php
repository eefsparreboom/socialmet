...
function optin_monster_ajax_get_optin_image( $optin_id, $size, $type, $theme ) {
    ...
    // Retrieve the alt text for the image and then return the image HTML.
    $alt = get_post_meta( $thumb_id, '_wp_attachment_image_alt', true );
    return '<img class="optin-monster-image optin-monster-image-' . $type . '-' . $theme . '" src="' . esc_url( $src[0] ) . '" alt="' . esc_attr( $alt ) . '" />';
