...
class Optin_Monster_License {
    ...
    public function verify_key() {
        ...
        $this->success[]       = isset( $verify->success ) ? $verify->success : __( 'Congratulations! This site is now receiving automatic updates.', 'optin-monster' );
        ...
        if ( false === ( $validate = get_transient( '_om_validate_license' ) ) ) {
            ...
            if ( ! $timestamp ) {
                ...
                if ( $current_timestamp < $timestamp ) {
                    ...
                    update_option( 'optin_monster_license_updates', strtotime( '+12 hours' ) );
                    $this->validate_key();
                }
            }
        }
    ...
    public function notices() {
        ...
        ?>
        <div class="error">
            <p><?php printf( __( 'No valid license key has been entered, so automatic updates for OptinMonster have been turned off. <a href="%s">Please click here to enter your license key and begin receiving automatic updates.</a>', 'optin-monster' ), add_query_arg( array( 'page' => 'optin-monster-settings#!optin-monster-tab-settings' ), admin_url( 'admin.php' ) ) ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php printf( __( 'Your license key for OptinMonster has expired. <a href="%s" target="_blank">Please click here to renew your license key and continue receiving automatic updates.</a>', 'optin-monster' ), 'https://optinmonster.com/login/' ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php _e( 'Your license key for Optin_Monster has been disabled. Please use a different key to continue receiving automatic updates.', 'optin-monster' ); ?></p>
        </div>
        <?php
        endif;
        ...
        ?>
        <div class="error">
            <p><?php _e( 'Your license key for OptinMonster is invalid. The key no longer exists or the user associated with the key has been deleted. Please use a different key to continue receiving automatic updates.', 'optin-monster' ); ?></p>
        </div>
        <?php
        endif;
